...
def create_interactive_dendrogram(truncate_mode='lastp', p=30, color_threshold=None):
    """Create interactive Plotly dendrogram"""
    global X_scaled, urls
    try:
        # Check if we have data
        if X_scaled is None or len(X_scaled) == 0:
            print("Warning: No data available for dendrogram")
            return None
            
        # Use a smaller sample for better performance and readability
        sample_size = min(1000, len(X_scaled))  # Limit to 1000 points max
        X_sample = X_scaled[:sample_size]
        
        # Create simple static dendrogram using matplotlib
        try:
            ...
            dendro = sch.dendrogram(
                ...
                color_threshold=25,
                truncate_mode='lastp',
                p=min(20, sample_size),  # Show last 20 clusters
                leaf_rotation=90,
                leaf_font_size=8
            )
            
            ax.set_title(f'Hierarchical Clustering Dendrogram (Sample: {sample_size} URLs)', 
                        fontsize=16, color='white', pad=20)
            ax.set_xlabel('Sample Index or (cluster size)', fontsize=12, color='white')
            ax.set_ylabel('Distance', fontsize=12, color='white')
            ...
            # Create a simple Plotly figure that displays the image
            fig = go.Figure()
            ...
            # Create a simple but informative scatter plot as fallback
            fig = go.Figure()
            
            # Create a 2D projection of the data for visualization
            from sklearn.decomposition import PCA
            pca = PCA(n_components=2)
            X_2d = pca.fit_transform(X_sample)
            
            # Color points by their cluster assignment
            colors = ['#4299e1'] * len(X_2d)  # Default blue
            
            fig.add_trace(go.Scatter(
                x=X_2d[:, 0],
                y=X_2d[:, 1],
                mode='markers',
                name='URL Data Points',
                marker=dict(
                    size=8, 
                    color=colors,
                    opacity=0.7
                ),
                hovertemplate='<b>URL %{text}</b><br>PC1: %{x:.2f}<br>PC2: %{y:.2f}<extra></extra>',
                text=[f"{i}" for i in range(sample_size)]
            ))
            
            fig.update_layout(
                title={
                    'text': f'URL Data Visualization (PCA Projection) - Sample: {sample_size} URLs',
                    'x': 0.5,
                    'xanchor': 'center',
                    'font': {'size': 18, 'color': '#e0e0e0'}
                },
                xaxis={
                    'title': {'text': 'First Principal Component', 'font': {'size': 14, 'color': '#e0e0e0'}},
                    'tickfont': {'color': '#a0aec0', 'size': 12},
                    'gridcolor': '#4a5568',
                    'linecolor': '#4a5568'
                },
                yaxis={
                    'title': {'text': 'Second Principal Component', 'font': {'size': 14, 'color': '#e0e0e0'}},
                    'tickfont': {'color': '#a0aec0', 'size': 12},
                    'gridcolor': '#4a5568',
                    'linecolor': '#4a5568'
                },
        ...
        print(f"Error creating interactive dendrogram: {e}")
        return None
...
def create_url_highlighted_dendrogram(url, truncate_mode='lastp', p=30, 
                                    color_threshold=None, leaf_rotation=90, 
                                    figsize=(12, 8), dpi=150):
    """Create dendrogram with URL position highlighted - shows where URL fits in clustering tree"""
    try:
        fig = plt.figure(figsize=figsize, dpi=dpi)
        ...
        # Find URL analysis info
        url_info = find_url_position_in_dendrogram(url)
        
        # Highlight the URL's position in the dendrogram
        if 'nearest_neighbors' in url_info and url_info['nearest_neighbors']:
            try:
                # Get the nearest neighbors from the analysis
                nearest_neighbors = url_info['nearest_neighbors']
                
                # Find which of these neighbors are visible in the current dendrogram
                visible_neighbors = []
                for neighbor in nearest_neighbors:
                    # Find the neighbor's index in the original dataset
                    try:
                        neighbor_idx = urls.index(neighbor['url'])
                        if neighbor_idx in dendro['leaves']:
                            visible_neighbors.append(neighbor_idx)
                    except ValueError:
                        # URL not found in training set
                        continue
                
                # Highlight visible neighbors with different colors based on their labels
                for i, neighbor_idx in enumerate(visible_neighbors):
                    leaf_pos = dendro['leaves'].index(neighbor_idx)
                    neighbor = next(nn for nn in nearest_neighbors if urls[neighbor_idx] == nn['url'])
                    
                    # Color code based on neighbor's label
                    if neighbor['label'] == 'phishing':
                        color = 'red'
                        marker = 'x'
                        size = 80
                    elif neighbor['label'] == 'legitimate':
                        color = 'green'
                        marker = 'o'
                        size = 60
                    else:
                    ...
                    plt.scatter(leaf_pos, 0, color=color, s=size, marker=marker, 
                              zorder=5, alpha=0.8, edgecolors='black', linewidth=1)
                
                # Add text annotation about the analysis
                if visible_neighbors:
                    plt.text(0.02, 0.98, f'URL Analysis: {len(visible_neighbors)}/{len(nearest_neighbors)} neighbors visible\nPrediction: {url_info.get("prediction", "unknown")}\nConfidence: {url_info.get("confidence", 0):.2f}', 
                            transform=plt.gca().transAxes, fontsize=9, 
                            bbox=dict(boxstyle="round,pad=0.3", facecolor="lightblue", alpha=0.8),
                            verticalalignment='top')
                else:
                    # If no neighbors are visible due to truncation, add a general indicator
                    plt.text(0.02, 0.98, f'URL Analysis: {len(nearest_neighbors)} neighbors found (truncated view)\nPrediction: {url_info.get("prediction", "unknown")}\nConfidence: {url_info.get("confidence", 0):.2f}', 
                            transform=plt.gca().transAxes, fontsize=9, 
                            bbox=dict(boxstyle="round,pad=0.3", facecolor="lightyellow", alpha=0.8),
                            verticalalignment='top')
            except Exception as e:
                # If there's an error with neighbor highlighting, just add a general message
                plt.text(0.02, 0.98, f'URL Analysis completed (highlighting unavailable)\nPrediction: {url_info.get("prediction", "unknown")}\nConfidence: {url_info.get("confidence", 0):.2f}', 
                        transform=plt.gca().transAxes, fontsize=9, 
                        bbox=dict(boxstyle="round,pad=0.3", facecolor="lightgray", alpha=0.8),
                        verticalalignment='top')
        
        plt.title(f'Clustering Tree with URL Pattern: {url[:50]}...', fontsize=18, fontweight='bold')  # Increased from 16
        plt.xlabel('Sample Index or (cluster size)', fontsize=14)  # Increased from 12
        plt.ylabel('Distance', fontsize=14)  # Increased from 12
        plt.axhline(y=10, color="r", linestyle="--", alpha=0.7, label='Distance Threshold (t=10)')
        plt.grid(True, alpha=0.3)
        
        # Add legend
        legend_elements = [
            plt.Line2D([0], [0], color='red', marker='x', linestyle='None', markersize=8, label='Suspicious Pattern Neighbors'),
            plt.Line2D([0], [0], color='green', marker='o', linestyle='None', markersize=6, label='Normal Pattern Neighbors'),
            plt.Line2D([0], [0], color='gray', marker='s', linestyle='None', markersize=6, label='Unknown Pattern Neighbors'),
            plt.Line2D([0], [0], color='red', lw=2, label='Suspicious Pattern Groups'),
            plt.Line2D([0], [0], color='green', lw=2, label='Normal Pattern Groups')
        ]
        plt.legend(handles=legend_elements, loc='upper right', fontsize=12)  # Increased font size
        ...
        # If there's any error, create a basic dendrogram without highlighting
        print(f"Warning: Error in URL highlighting: {e}")
        fig = plt.figure(figsize=figsize, dpi=dpi)
        ...
        plt.title(f'Dendrogram: {url[:50]}...', fontsize=16, fontweight='bold')
...
def get_dendrogram():
    """Get dendrogram with custom parameters"""
    try:
        truncate_mode = request.args.get('truncate_mode', 'lastp')
        p = int(request.args.get('p', 30))
        color_threshold = request.args.get('color_threshold')
        color_threshold = float(color_threshold) if color_threshold else None
        leaf_rotation = int(request.args.get('leaf_rotation', 90))
        
        fig = create_dendrogram_figure(
            truncate_mode=truncate_mode,
            p=p,
            color_threshold=color_threshold,
            leaf_rotation=leaf_rotation
        ...
        return jsonify({'dendrogram': f'data:image/png;base64,{image_b64}'})
...
@app.route('/export')
def export_data():
    """Export clustering data as JSON"""
    try:
        export_data = {
            'timestamp': datetime.now().isoformat(),
            'model_info': {
                'n_samples': len(urls) if urls else 0,
                'n_clusters': len(set(clusters)) if clusters is not None else 0,
                'cluster_map': cluster_map,
                'features': list(extract_features('https://example.com').keys())
            },
            'clustering_results': {
                'urls': urls[:100] if urls else [],  # Limit for performance
                'labels': labels[:100] if labels else [],
                'clusters': clusters[:100].tolist() if clusters is not None else []
            }
        }
...
def create_interactive_url_analysis(url, url_info, clusters, labels):
    """Create interactive URL cluster analysis chart"""
    try:
        # Validate input data
        ...
        for i, cluster in enumerate(clusters):
            # Skip invalid cluster values
            if cluster is None or (isinstance(cluster, (int, float)) and cluster < 0):
                continue
        ...
        # Determine cluster types and colors
        cluster_types = {}
        for cluster, labels_list in cluster_labels.items():
            if labels_list:
                label_counts = pd.Series(labels_list).value_counts()
        ...
        # Create colors array in the correct order
        colors = []
        for cluster in clusters_sorted:
            cluster_type = cluster_types.get(cluster, 'unknown')
            if cluster_type == 'phishing':
                colors.append('#f56565')  # Red for suspicious
            elif cluster_type == 'legitimate':
                colors.append('#48bb78')  # Green for normal
            else:
                colors.append('#718096')  # Gray for unknown
        ...
        fig = go.Figure(data=[
            go.Bar(
                ...
                text=counts,
                textposition='auto',
                hovertemplate='<b>Cluster %{x}</b><br>Count: %{y}<br>Type: %{customdata}<extra></extra>',
                customdata=[cluster_types.get(c, 'Unknown') for c in clusters_sorted]
            )
        ])
        ...
        if cluster_id != 'N/A' and cluster_id in clusters_sorted:
            fig.add_vline(
                ...
                line_dash="dash",
                line_color="#63b3ed",
                annotation_text=f"URL Cluster: {cluster_id}",
        ...
        fig.update_layout(
            title={
                'text': f'URL Pattern Analysis: {url[:50]}...<br>Pattern Group: {url_info.get("prediction", "unknown")} | Similarity: {url_info.get("confidence", 0):.2f} | Cluster: {cluster_id}',
                'x': 0.5,
                'xanchor': 'center',
                'font': {'size': 16, 'color': '#e0e0e0'}
            },
            xaxis={
                'title': {'text': 'Cluster ID', 'font': {'size': 14, 'color': '#e0e0e0'}},
                'tickfont': {'color': '#a0aec0'},
                'gridcolor': '#4a5568',
                'linecolor': '#4a5568'
            },
            yaxis={
                'title': {'text': 'Number of Samples', 'font': {'size': 14, 'color': '#e0e0e0'}},
                'tickfont': {'color': '#a0aec0'},
                'gridcolor': '#4a5568',
                'linecolor': '#4a5568'
            },
            ...
            paper_bgcolor='#2d3748',
            font={'color': '#e0e0e0'},
            height=600,  # Increased height to match dendrogram
        ...
        print(f"Error creating interactive URL analysis: {e}")
        return None
